...
/** POST /api/exercises/:id/reset-submission — deshace la aprobación previa (quita XP) para poder reintentar sin duplicar. */
async function resetSubmission(req, res) {
  ...
  const result = await progressService.undoExerciseCompletion({ userId, exerciseId: exercise.id });
  res.json(result);
}
...
module.exports = { listExercises, getExercise, runCode, submit, generate, getTodayChallenge, resetSubmission };